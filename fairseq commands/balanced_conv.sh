...
fairseq-preprocess --source-lang en --target-lang fr --testpref test --tokenizer moses --bpe subword_nmt --destdir %MODEL_DIR%/data-bin/europarl-v7.fr-en.balanced_conv --srcdict %MODEL_DIR%/data-bin/wmt14.en-fr.fconv-py/dict.en.txt --tgtdict %MODEL_DIR%/data-bin/wmt14.en-fr.fconv-py/dict.fr.txt --workers 8
...
fairseq-generate data-bin/europarl-v7.fr-en.balanced_conv --path data-bin/wmt14.en-fr.fconv-py/model.pt --beam 3 --batch-size 32 --source-lang en --target-lang fr --gen-subset test --tokenizer moses --bpe subword_nmt --bpe-codes data-bin/wmt14.en-fr.fconv-py/bpecodes > data-bin/out_balanced_conv.txt
